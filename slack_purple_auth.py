#!/usr/bin/env python3
#
# Derived from extract_token_from_browser.py in wee-slack
# License: MIT
...
parser.add_argument(
    "username",
    help="Which username to extract token for",
    metavar="<username>",
)
parser.add_argument(
...
url = "https://" + args.username.split("%")[1] + "/"
for team in local_config["teams"].values():
    if team["id"].startswith("E"):
        continue
    if team["url"] == url:
        print("%s %s"% (team["token"], cookie_value))
